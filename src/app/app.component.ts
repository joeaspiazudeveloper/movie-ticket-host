...
export class AppComponent {
  ...
  async ngOnInit() {
    try {
      // 1. Load movieapp
      const movieappModule = await this.microfrontendService.loadRemoteComponent(4201, 'movieapp');
      ...
      // 4. Load ticketapp
      const ticketappModule = await this.microfrontendService.loadRemoteComponent(4202, 'ticketapp');
